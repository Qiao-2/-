...
function unlockAdminByPassword(password) {
...
els.dialogSubmitBtn.addEventListener('click', () => {
  const ok = unlockAdminByPassword(els.dialogPasswordInput.value);
  if (ok && els.passwordDialog?.open) els.passwordDialog.close();
});
els.adminPasswordInput.addEventListener('keydown', (e) => {
  if (e.key === 'Enter') {
    const ok = unlockAdminByPassword(els.adminPasswordInput.value);
